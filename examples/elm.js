...
Elm.Collision.Hitbox = Elm.Collision.Hitbox || {};
Elm.Collision.Hitbox.make = function (_elm) {
   "use strict";
   _elm.Collision = _elm.Collision || {};
   _elm.Collision.Hitbox = _elm.Collision.Hitbox || {};
   if (_elm.Collision.Hitbox.values)
   return _elm.Collision.Hitbox.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var circle = F3(function (centerX,centerY,radius) {
      return {cx: centerX,cy: centerY,radius: radius};
   });
   var Circle = F3(function (a,b,c) {
      return {cx: a,cy: b,radius: c};
   });
   var rectangle = F4(function (centerX,centerY,width,height) {
      return {cx: centerX,cy: centerY,w: width,h: height};
   });
   var Rectangle = F4(function (a,b,c,d) {
      return {cx: a,cy: b,w: c,h: d};
   });
   return _elm.Collision.Hitbox.values = {_op: _op
                                         ,rectangle: rectangle
                                         ,circle: circle
                                         ,Rectangle: Rectangle
                                         ,Circle: Circle};
...
Elm.Collision.make = function (_elm) {
   "use strict";
   _elm.Collision = _elm.Collision || {};
   if (_elm.Collision.values) return _elm.Collision.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Collision$Hitbox = Elm.Collision.Hitbox.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   var Top = {ctor: "Top"};
   var rectangleSide = F2(function (rect1,rect2) {
      var dy = rect2.cy - rect1.cy;
      var dx = rect2.cx - rect1.cx;
      var h = 0.5 * (rect1.h + rect2.h);
      var hx = h * dx;
      var w = 0.5 * (rect1.w + rect2.w);
      var wy = w * dy;
      return _U.cmp($Basics.abs(dx),w) < 1 && _U.cmp($Basics.abs(dy),
      h) < 1 ? _U.cmp(wy,hx) > 0 ? _U.cmp(wy,
      0 - hx) > 0 ? $Maybe.Just(Top) : $Maybe.Just(Left) : _U.cmp(wy,
      0 - hx) > 0 ? $Maybe.Just(Right) : $Maybe.Just(Bottom) : $Maybe.Nothing;
   });
   var circleToCircle = F2(function (circle1,circle2) {
      var dy = circle1.cy - circle2.cy;
      var dx = circle1.cx - circle2.cx;
      var distance = $Basics.sqrt(dx * dx + dy * dy);
      return _U.cmp(distance,
      circle1.radius + circle2.radius) < 0 ? true : false;
   });
   var axisAlignedBoundingBox = F2(function (rect1,rect2) {
      var startingPoint = F2(function (centerPoint,length) {
         return centerPoint - length / 2;
      });
      var x1 = A2(startingPoint,rect1.cx,rect1.w);
      var x2 = A2(startingPoint,rect2.cx,rect2.w);
      var y1 = A2(startingPoint,rect1.cy,rect1.h);
      var y2 = A2(startingPoint,rect2.cy,rect2.h);
      return _U.cmp(x1,x2 + rect2.w) < 0 && (_U.cmp(x1 + rect1.w,
      x2) > 0 && (_U.cmp(y1,y2 + rect2.h) < 0 && _U.cmp(rect1.h + y1,
      y2) > 0)) ? true : false;
   });
   return _elm.Collision.values = {_op: _op
...
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Collision = Elm.Collision.make(_elm),
   $Collision$Hitbox = Elm.Collision.Hitbox.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Collage = Elm.Graphics.Collage.make(_elm),
   ...
   var _op = {};
   var message = F2(function (title,stringable) {
      return $Graphics$Collage.toForm($Graphics$Element.show(A2($Basics._op["++"],
      title,
      A2($Basics._op["++"],": ",$Basics.toString(stringable)))));
   });
   var scene = F2(function (_p1,_p0) {
      var _p2 = _p1;
      var _p3 = _p0;
      var _p6 = _p3._0;
      var _p5 = _p3._1;
      var rectangle2Hitbox = A4($Collision$Hitbox.rectangle,
      0,
      0,
      80,
      80);
      var _p4 = {ctor: "_Tuple2"
                ,_0: $Basics.toFloat(_p2._0) - $Basics.toFloat(_p6) / 2
                ,_1: $Basics.toFloat(_p5) / 2 - $Basics.toFloat(_p2._1)};
      var dx = _p4._0;
      var dy = _p4._1;
      var rectangle1Hitbox = A4($Collision$Hitbox.rectangle,
      dx,
      dy,
      60,
      60);
      var rectanglesCollision = A2($Collision.rectangleSide,
      rectangle1Hitbox,
      rectangle2Hitbox);
      var rectangleColor = !_U.eq(rectanglesCollision,
      $Maybe.Nothing) ? $Color.red : $Color.blue;
      return A3($Graphics$Collage.collage,
      _p6,
      _p5,
      _U.list([A2($Graphics$Collage.filled,
              rectangleColor,
              A2($Graphics$Collage.rect,80,80))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: dx,_1: dy},
              A2($Graphics$Collage.filled,
              $Color.orange,
              A2($Graphics$Collage.rect,60,60)))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: dx,_1: dy + 10},
              A2(message,"Side",rectanglesCollision))]));
   });
